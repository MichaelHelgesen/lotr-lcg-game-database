...
export const CardListDeckComponent = React.forwardRef((props, ref) => {
    const { card, cardList, deckList, quantity, traitsList, setDeckList, value, onChange, size, sphereList } = props
    ...
    return (
        <Card padding={0}>
            <Box>
                <CardTooltip card={card} quantity={quantity} setOpen={setOpen} size={size} traitsList={traitsList} />
            </Box>
